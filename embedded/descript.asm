...
;
; File created: 15/01/2010 17:45:38
...
;   #include p18f2450.inc
  ...
  extern InVarDirection
  extern InVarLSBCounter
  extern OutVarSpeed
  extern InVarMSBCounter
  extern OutVarDirection
...
DeviceDescriptor:
 global DeviceDescriptor
    ...
    retlw  0xD8   ;Vendor ID (2 hex bytes)
    retlw  0x04
    retlw  0x00   ;Product ID (2 hex bytes)
    retlw  0x20
    retlw  0x00   ;Device release number (2 hex bytes)
    retlw  0x00
...
Report1:
   ...
   retlw  0x09   ;  Usage (48),
   retlw  0x30
   retlw  0x81   ;  Input (Data, Variable, Absolute)   ; InVarDirection
   retlw  0x02
   retlw  0x09   ;  Usage (49),
   retlw  0x31
   retlw  0x81   ;  Input (Data, Variable, Absolute)   ; InVarLSBCounter
   retlw  0x02
   retlw  0x09   ;  Usage (62),
   retlw  0x3E
   retlw  0x91   ;  Output (Data, Variable, Absolute)   ; OutVarSpeed
   retlw  0x02
   retlw  0x09   ;  Usage (50),
   retlw  0x32
   retlw  0x81   ;  Input (Data, Variable, Absolute)   ; InVarMSBCounter
   retlw  0x02
   retlw  0x09   ;  Usage (63),
   retlw  0x3F
   retlw  0x91   ;  Output (Data, Variable, Absolute)   ; OutVarDirection
   retlw  0x02
...
Ep1RcvVarTable:
  global Ep1RcvVarTable
    db  0x88, low (OutVarSpeed)        ; OutVarSpeed starts in 1st byte
    db  high (OutVarSpeed), 0x88        ; OutVarDirection starts in 2nd byte
    db  low (OutVarDirection), high (OutVarDirection)
    db  0x00, 0x00        ; VarTable ends in 1st byte


Ep1XmtVarTable:
  global Ep1XmtVarTable
    db  0x88, low (InVarDirection)        ; InVarDirection starts in 1st byte
    db  high (InVarDirection), 0x88        ; InVarLSBCounter starts in 2nd byte
    db  low (InVarLSBCounter), high (InVarLSBCounter)
    db  0x88, low (InVarMSBCounter)        ; InVarMSBCounter starts in 1st byte
    db  high (InVarMSBCounter), 0x00        ; VarTable ends in 2nd byte
