...
namespace Tour1
{
    ...
    private void MainForm_Load(object sender, System.EventArgs e)
    {
      ...
      this.Text = "Test program for HIDmaker project: " + TTourDev.HM_PROJECT_NAME;
    ...
    private bool OpenOurHid()
    {
      ...
      do
      {
        axHIDagentX1.VID_To_Match = TTourDev.MY_VID;
        axHIDagentX1.PID_to_Match = TTourDev.MY_PID;
    ...
    private TTourDev LatestTourDev()
    {
      if (TourDev.Count > 0)
        return (TTourDev)TourDev[TourDev.Count - 1];
      else
        return null;
    }  // private TTourDev Latesttest1()
    //---------------------------------------------------------------------------
    ...
    private void axHIDagentX1_OnOpenIntf(object sender, AxHIDagentXControl1.IHIDagentXEvents_OnOpenIntfEvent e)
    {
      ...
      // in our TTourDev object.  This means we have to call the
      // TTourDev.Startup method when each of the associated Interfaces is
      ...
      IsNew = true;
      if (LatestTourDev() == null)
        aTourDev = new TTourDev(); // 1st new device: no Ttest1 objects yet
      else
        {
          if (LatestTourDev().UnboundVarCount() == 0)
              aTourDev = new TTourDev();  // <-- new device
            else
              {
                aTourDev = LatestTourDev();  // <-- still need to bind more vars in this device
                IsNew = false;
              }
        }

      // See if this newly-opened HID interface is expected by TTourDev object
      // If device is unknown, BindVars returns -1, so we can discard aTourDev object
        if (aTourDev.BindVars(axHIDagentX1) >= 0)
        {
          // Yes, this is a known device. If aTourDev is newly created, add it to
          // the list and hook up its events
          if (IsNew)
            {
              TourDev.Add(aTourDev);

              // Connect events from HIDagent to aTourDev object
              aTourDev.BindEvents();
            }
        }
    ...
    private void axHIDagentX1_OnCloseIntf(object sender, AxHIDagentXControl1.IHIDagentXEvents_OnCloseIntfEvent e)
    {
      ...
      for (i = TourDev.Count - 1; i >= 0; i--)
        {
          ATourDev = (TTourDev)TourDev[i];
          if (ATourDev.HasHandle(e.intfHandle))
            {
              ATourDev.Shutdown();
              TourDev.RemoveAt(i);
    ...
    private void ReadRptsBtn_Click(object sender, System.EventArgs e)
    {
      ...
      for (DevNum = 0; DevNum < TourDev.Count; DevNum++)
      {
        AddToResults("");
        AStr = "Device #" + DevNum.ToString() + ":";
        AddToResults(AStr);
        if ( ((TTourDev)TourDev[DevNum]).ReadAllReports() )
        {
          AStr = "  InVar1 = " + ((TTourDev)TourDev[DevNum]).InVar1.UnScaledValue.ToString();
          AddToResults(AStr);

          AStr = "  InVar2 = " + ((TTourDev)TourDev[DevNum]).InVar2.UnScaledValue.ToString();
          AddToResults(AStr);
    ...
    private void SendRptsBtn_Click(object sender, System.EventArgs e)
    {
      ...
      ((TTourDev)TourDev[DevNum]).OurVar1.UnScaledValue = 39;
      // Activate next 2 lines to send random values
      //MaxVal = 127;
      //((TTourDev)TourDev[DevNum]).OurVar1.UnScaledValue = RandomNum.Next(MaxVal+1);
      AStr = "  Writing to device: OurVar1 = 0x" + 
        ((TTourDev)TourDev[DevNum]).OurVar1.UnScaledValue.ToString("x");
      AddToResults(AStr);

            if (((TTourDev)TourDev[DevNum]).WriteAllReports())
            {
              // Success
